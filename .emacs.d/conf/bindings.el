...
(define-key global-map "\ef" 'forward-word-stop-at-uppercase)
...
(define-key global-map "\^l" 'recenter) ;;(tcode-transpose-strokes)
...
(define-key global-map "\C-cm" 'compare-windows)
(defun compare-windows-ignore-whitespace ()
  (interactive)
  (let ((compare-ignore-whitespace t))
    (compare-windows nil)))
(define-key global-map "\C-cw" 'compare-windows-ignore-whitespace)
(define-key global-map "\C-c\C-w" 'compare-windows-ignore-whitespace)
(fset 'compare-windows-dehighlight 'compare-windows-ignore-whitespace)
(define-key global-map "\C-cv" 'view-mode)
